...
void read_workqueue_fn(struct work_struct *work);
void  write_workqueue_fn(struct work_struct *work);
...
void read_workqueue_fn(struct work_struct *work)
{	
	//s32 sec, min, hour, day, mon, year,date,s,m,hr,d,mn,y,wday;
	printk(KERN_INFO "Executing Read Workqueue Function\n");
...
void  write_workqueue_fn(struct work_struct *work)	
{
	...
	printk(KERN_INFO "Executing  Write Workqueue Function\n");
...
static int __init ds1307_entry(void)
{
	...
	if(alloc_chrdev_region(&ds1307_dev,0,1,"wq_ds_1307")<0)
	{
	...
	if(IS_ERR(ds1307_class =class_create(THIS_MODULE, "wq_ds1307_class")))
	{
		pr_info("Cannot create the struct class\n");
		goto r_class;
	}
	if(IS_ERR(device_create(ds1307_class,NULL,ds1307_dev,NULL,"wq_ds1307_dev")))
	{
	...
	if (request_irq(IRQ_NO, irq_handler, IRQF_SHARED, "wq_ds_1307", (void *)(irq_handler))) {
		printk(KERN_INFO "my_device: cannot register IRQ ");
		goto irq;
	}

	INIT_WORK(&read_work, read_workqueue_fn);
	INIT_WORK(&write_work, write_workqueue_fn);
	...
	if (!rtc)
		return -ENOMEM;
#if 1
	rtc = devm_rtc_device_register(&ds1307_client->dev,"wq_ds1307",&ds1307_rtc_ops, THIS_MODULE);
	if (IS_ERR(rtc))
	{       
		printk(KERN_DEBUG "Here I am: %s:%i\n", __FUNCTION__, __LINE__);
		return PTR_ERR(rtc);
