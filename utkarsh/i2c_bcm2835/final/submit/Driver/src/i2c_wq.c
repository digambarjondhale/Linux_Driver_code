...
struct bcm2835_i2c_dev *mydev;
static inline void bcm2835_i2c_writel(struct bcm2835_i2c_dev *i2c_dev, u32 reg, u32 val){
...
static int clk_bcm2835_i2c_calc_divider(unsigned long rate, unsigned long parent_rate){
...
static int clk_bcm2835_i2c_set_rate(struct clk_hw *hw, unsigned long rate, unsigned long parent_rate){
...
static long clk_bcm2835_i2c_round_rate(struct clk_hw *hw, unsigned long rate, unsigned long *parent_rate){
	u32 divider = clk_bcm2835_i2c_calc_divider(rate, *parent_rate);

	return DIV_ROUND_UP(*parent_rate, divider);
}

static unsigned long clk_bcm2835_i2c_recalc_rate(struct clk_hw *hw, unsigned long parent_rate){
	struct clk_bcm2835_i2c *div = to_clk_bcm2835_i2c(hw);
...
static struct clk *bcm2835_i2c_register_div(struct device *dev, struct clk *mclk, struct bcm2835_i2c_dev *i2c_dev){
...
/* workqueue function */
void workqueue_fn(struct work_struct *work){
	pr_info("Inside workqueue\n");
	bcm2835_i2c_start_transfer(mydev);
}
...
static irqreturn_t bcm2835_i2c_isr(int this_irq, void *data){
	...
	if (val & BCM2835_I2C_S_TXW) {
		...
		if (i2c_dev->num_msgs && !i2c_dev->msg_buf_remaining) {
			i2c_dev->curr_msg++;
			mydev = i2c_dev;
			/* scheduling the work to workqueue */
			schedule_work(&tf_work);
...
static int bcm2835_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num){
...
static int bcm2835_i2c_probe(struct platform_device *pdev){
	...
	/* initializing workqueue by dynamic method */
	INIT_WORK(&tf_work, workqueue_fn);
