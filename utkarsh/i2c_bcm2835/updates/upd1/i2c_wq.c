...
void i2c_start_transfer_wq_fn(struct work_struct *work);
void fill_txfifo_wq_fn(struct work_struct *work);
void drain_rxfifo_wq_fn(struct work_struct *work);
...
struct i2c_work_data{
	struct work_struct i2c_start_transfer_work;
	struct work_struct fill_txfifo_work;
	struct work_struct drain_rxfifo_work;
	struct bcm2835_i2c_dev *i2c_dev;
};
//struct bcm2835_i2c_dev *i2c_dev;
struct i2c_work_data *work_data;
...
void fill_txfifo_wq_fn(struct work_struct *work){
	work_data = container_of(work, struct i2c_work_data, fill_txfifo_work);
	struct bcm2835_i2c_dev *i2c_dev = work_data->i2c_dev;
	pr_info("Inside fill txfifo workqueue\n");
	bcm2835_fill_txfifo(i2c_dev);
}

void drain_rxfifo_wq_fn(struct work_struct *work){
	work_data = container_of(work, struct i2c_work_data, drain_rxfifo_work);
	struct bcm2835_i2c_dev *i2c_dev = work_data->i2c_dev;
	pr_info("Inside drain rxfifo workqueue\n");
	bcm2835_drain_rxfifo(i2c_dev);
}

void i2c_start_transfer_wq_fn(struct work_struct *work){
	struct i2c_work_data *work_data = container_of(work, struct i2c_work_data, i2c_start_transfer_work);
	struct bcm2835_i2c_dev *i2c_dev = work_data->i2c_dev;
	pr_info("Inside start transfer workqueue\n");
...
static irqreturn_t bcm2835_i2c_isr(int this_irq, void *data)
{
	...
	if (val & BCM2835_I2C_S_DONE) {
		if (!i2c_dev->curr_msg) {
			...
			schedule_work(&work_data->drain_rxfifo_work);
			//bcm2835_drain_rxfifo(i2c_dev);
			val = bcm2835_i2c_readl(i2c_dev, BCM2835_I2C_S);
		}
	...
	if (val & BCM2835_I2C_S_TXW) {
		...
		schedule_work(&work_data->fill_txfifo_work);
		//bcm2835_fill_txfifo(i2c_dev);

		if (i2c_dev->num_msgs && !i2c_dev->msg_buf_remaining) {
			//struct i2c_work_data *work_data;
			i2c_dev->curr_msg++;
			//work_data = kmalloc(sizeof(struct i2c_work_data), GFP_KERNEL);
				//work_data->i2c_dev = i2c_dev;
				schedule_work(&work_data->i2c_start_transfer_work);
			//bcm2835_i2c_start_transfer(i2c_dev);
		}
...
static int bcm2835_i2c_probe(struct platform_device *pdev)
{
	...
	work_data = kmalloc(sizeof(struct work_struct), GFP_KERNEL);
	if(!work_data){
		printk(KERN_INFO "cannot allocate memory");
		goto err_disable_unprepare_clk;
	}
	INIT_WORK(&work_data->i2c_start_transfer_work, i2c_start_transfer_wq_fn);
	INIT_WORK(&work_data->fill_txfifo_work, fill_txfifo_wq_fn);
	INIT_WORK(&work_data->drain_rxfifo_work, drain_rxfifo_wq_fn);
	return 0;
...
static int bcm2835_i2c_remove(struct platform_device *pdev)
{
	...
	flush_work(&work_data->i2c_start_transfer_work);
	flush_work(&work_data->fill_txfifo_work);
	flush_work(&work_data->drain_rxfifo_work);	
	clk_rate_exclusive_put(i2c_dev->bus_clk);
	clk_disable_unprepare(i2c_dev->bus_clk);
	free_irq(i2c_dev->irq, i2c_dev);
