...
struct bcm2708_i2c *my_bi;
struct tasklet_struct *mytasklet = NULL; 
void mytasklet_fn(unsigned long);
bool handled = true;
...
/*Tasklet Function*/
void mytasklet_fn(unsigned long arg) 
{
        printk(KERN_INFO "Executing Tasklet Function\n");
	u32 s;
	int ret;

	spin_lock(&my_bi->lock);

	/* we may see camera interrupts on the "other" I2C channel
		   Just return if we've not sent anything */
	if (!my_bi->nmsgs || !my_bi->msg) {
		goto early_exit;
	}

	s = bcm2708_rd(my_bi, BSC_S);

	if (s & (BSC_S_CLKT | BSC_S_ERR)) {
		bcm2708_bsc_reset(my_bi);
		my_bi->error = true;

		my_bi->msg = 0; /* to inform the that all work is done */
		my_bi->nmsgs = 0;
		/* wake up our bh */
		complete(&my_bi->done);
		...
		if (my_bi->msg->flags & I2C_M_RD) {
			bcm2708_bsc_fifo_drain(my_bi);
		}
...
{
        printk(KERN_INFO "IRQ: Interrupt Occurred");
	struct bcm2708_i2c *my_bi = dev_id;
        /*Allocating tasklet*/
...
static int bcm2708_i2c_probe(struct platform_device *pdev)
{
	...
	if (err) {
        ...
        mytasklet  = kmalloc(sizeof(struct tasklet_struct),GFP_KERNEL);
        if(mytasklet == NULL) {
            printk(KERN_INFO "cannot allocate Memory");
		goto out_iounmap;
        }
        tasklet_init(mytasklet,mytasklet_fn,0);
